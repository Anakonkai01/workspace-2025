...
class TrafficSignConfig:
    def __init__(self):
        ...
        self.USE_MULTIPROCESSING = False  # Disabled to prevent memory overflow
        self.NUM_WORKERS = 4  # Parallel workers for detection
        self.BATCH_SIZE = 50   # Frames per batch
        self.CACHE_FRAMES = False  # Disabled - streaming mode
        ...
        self.SHAPE_PARAMS = {
            'circle': {
                ...
                'trust_threshold': 1100,
                'small_circularity': 0.80,
                'large_circularity': 0.87
            },
            'triangle': {
...
# =============================================================================
# CLASS 5: SVM DETECTOR (MODEL B: Sign/Not-Sign)
# =============================================================================
class SignDetectorSVM:
    def __init__(self, model_path: str):
        print(f"Đang tải mô hình SVM Detector từ: {model_path}")
        try:
            self.svm = cv2.ml.SVM_load(model_path)
            if self.svm is None: 
                raise Exception(f"Không thể tải model từ {model_path}")
        except Exception as e:
            print(f"❌ Lỗi nghiêm trọng: Không thể tải mô hình Detector.")
            print(f"   Chi tiết: {e}")
            raise
            
        # Cấu hình HOG phải GIỐNG HỆT khi huấn luyện
        RESIZE_DIM = (64, 64)
        winSize = RESIZE_DIM
        blockSize = (16, 16)
        blockStride = (8, 8)
        ...
        print("✓ Tải mô hình SVM Detector thành công.")

    def verify(self, frame: np.ndarray, bbox: Tuple) -> bool:
        """Kiểm tra xem bbox có thực sự là biển báo không."""
        x, y, w, h = bbox
        ...
        try:
            # 1. Cắt (crop) vùng biển báo
            sign_roi = frame[y:y+h, x:x+w]
            
            # 2. Tiền xử lý: Chuyển xám và Resize
            ...
            # 1 là "Sign", 0 là "Not-Sign"
            return label == 1
            
        except Exception as e:
            # Bỏ qua lỗi (ví dụ: bbox nằm ngoài biên)
...
def main():
    ...
    try:
        ...
        # =====================================================================
        # PASS 1: DETECTION (STREAMING - NO MEMORY LOADING)
        # =====================================================================
        print("\n" + "=" * 70)
        ...
        # Process frames DIRECTLY without storing in memory
        print(f"Processing frames in streaming mode (up to {config.MAX_FRAME_ID})...")
        detector = TrafficSignDetector(config)
        frame_count = 0
        all_masks_results = []  # Only store masks if needed
        ...
        while cap.isOpened() and frame_count < config.MAX_FRAME_ID:
            ...
            # Process frame immediately (no storage)
            detections, masks = detector.process_frame(frame_to_process, full_frame_dims)
            
            # Add detections to temporal filter directly
            if detections:
                temporal_filter.add_detections(frame_count, detections)
            
            # Only store masks if needed for video generation
            if config.SAVE_MASK_VIDEOS:
                all_masks_results.append((frame_count, masks))
            
            frame_count += 1
            
            if frame_count % config.PROGRESS_UPDATE_INTERVAL == 0:
                progress = int(frame_count / config.MAX_FRAME_ID * 100) if config.MAX_FRAME_ID > 0 else 0
        ...
        while cap.isOpened():
            ...
            validated_candidates = temporal_filter.get_validated_detections(frame_count_render)
            
            
            # filter candidates using svm detector 
            final_detections = []
